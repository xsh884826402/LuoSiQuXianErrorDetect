...
def get_data():#读取数据，根据cos值构造相似矩阵
    ...
    unidata = pd.read_excel('D:/距离测试/13Ddatauni2000.xlsx', header=None, skiprows=None)  # 训练数据文件
...
def spectral_cluster():#谱聚类
    ...
    for i in range(len(sc.labels_)):
        srcfile = 'D:/距离测试/13D归一化图像/' + str(i + 1) + '.jpeg'
        dstfile = 'D:/距离测试/13D分类/' + str(sc.labels_[i]) + '/' + str(i + 1) + '.jpeg'
